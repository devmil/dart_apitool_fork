    ...
    await Future.forEach<SourceItem>(sources, (sourceItem) async {
      if (sourceItem.sourceDir.startsWith('GIT:')) {
        ...
        if (!sourceItem.isInCache) {
          ...
          // A pub-cache entry can contain build state from an earlier local
          // analysis. Its package config points back to the cache and makes
          // api_summary treat declarations in this temporary copy as external.
          ...
          if (await dartToolDirectory.exists()) {
            await dartToolDirectory.delete(recursive: true);
          }
